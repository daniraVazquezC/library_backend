...
* **Production** - [https://library_backend-production.herokuapp.com](https://library_backend-production.herokuapp.com)
* **Staging** - [https://library_backend-staging.herokuapp.com](https://library_backend-staging.herokuapp.com)
...
* [Github repo](https://github.com/IcaliaLabs/library_backend)
...
```bash
$ git clone git@github.com:IcaliaLabs/library_backend.git
...
That command will lift every service library_backend needs, such as the `rails server`, `postgres`, and `redis`.
...
In order to stop library_backend as a whole you can run:
...
You probably won't be working with a blank database, so once you are able to run library_backend you can restore the database, to do it, first stop all services:
...
```
% docker exec -ti library_backend_db_1 bash
```

This will open up a bash session in to the database container.

Up to this point we just need to download a database dump and copy under `library_backend/backups/`, this directory is mounted on the container, so you will be able to restore it with:

```
root@a3f695b39869:/# bin/restoredb library_backend_dev db/backups/<databaseDump>
```
